...
type
  TDrawTextJustifiedOptions = set of (tjJustifyTrailingLines, tjMeasureOnly, tjForceManual);
...
procedure ManualJustifyTextOut(const Canvas : TCanvas; const X, Y : Integer; const Line : TLine;
  const SpaceToFill : Integer; const BreakChar : Char);
var
...
begin
  if not Line.Justify then begin
    TextOut(Canvas.Handle, X, Y, PChar(Line.Text), Length(Line.Text));
    Exit;
  end;
  ...
  // the inbuilt Winodws method; toggling between manual and inbuilt will shift mid-line words by
  ...
  try
    ...
    Words := SplitString(Line.Text, BreakChar);
...
procedure DrawLines(const Canvas : TCanvas; var Rect : TRect; const Lines : TList<TLine>;
  const MeasureOnly : Boolean; const ForceManual : Boolean);
var
  LineHeight,
  Index : Integer;
  SupportsJustification : Boolean;
    ...
    if SupportsJustification and not ForceManual then begin
      ...
      for Index := 0 to Pred(Lines.Count) do begin
        ManualJustifyTextOut(Canvas, Rect.Left, Rect.Top + Index * LineHeight, Lines[Index],
          Rect.Width - Lines[Index].FLengthPx, BreakChar);
      end;
    end;
  end;
end;
